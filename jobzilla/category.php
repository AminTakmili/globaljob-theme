<?php 
	...
	$total_post_count = $queried_object_data->count;
	$category_name = $queried_object_data->name;
	
	$page_title = (!empty($page_title)) ? $page_title : esc_html__('Category : ', 'jobzilla');
	$page_title .='<span>' . $queried_object_data->name. '</span>';	
				...
				<div class="<?php echo esc_attr($layout_class);?>">
					...
					<?php 
						...
						{ 
							...
							{
								get_template_part('dz-inc/elements/category_page_ajax_posts_element');
							}
							else
							{					
								get_template_part('dz-inc/elements/category_page_pagination_posts_element');
							}
						}
						else
						{
