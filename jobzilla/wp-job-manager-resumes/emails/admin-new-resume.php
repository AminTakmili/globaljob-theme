...
echo wp_kses_post( sprintf( __( 'A new resume has been submitted to <a href="%1$s">%2$s</a>.', 'jobzilla' ), home_url(), get_bloginfo( 'name' ) ) );

switch ( $resume->post_status ) {
	case 'publish':
		echo ' ' . esc_html__( 'It has been published and is now available to the public.', 'jobzilla' );
		break;
	case 'pending':
		// translators: Placeholder is URL for WP admin.
		echo ' ' . wp_kses_post( sprintf( __( 'It is awaiting approval by an administrator in <a href="%s">WordPress admin</a>.', 'jobzilla' ), esc_url( admin_url( 'edit.php?post_type=resume' ) ) ) );
		break;
}
