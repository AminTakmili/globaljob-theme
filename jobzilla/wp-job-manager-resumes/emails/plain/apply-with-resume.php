...
echo esc_html(
	sprintf(
		...
		__( 'A candidate has applied on the site %1$s (%2$s) for the position %3$s (%4$s).', 'jobzilla' ),
		get_bloginfo( 'name' ),
		esc_url( home_url() ),
		get_the_title( $job ),
...
if ( ! empty( $args['resume_link'] ) ) {
	...
	echo esc_html( sprintf( __( 'You can view their online resume by visiting %s.', 'jobzilla' ), $args['resume_link'] ) );
}
...
if ( ! empty( $candidate_email ) ) {
	...
	echo esc_html( sprintf( __( ' Additionally, you can contact them directly at %s.', 'jobzilla' ), $candidate_email ) );
}

echo PHP_EOL . PHP_EOL;

echo esc_html__( 'They included the following message along with their resume: ', 'jobzilla' ) . PHP_EOL;
