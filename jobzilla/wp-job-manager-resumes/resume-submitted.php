		...
		if ( resume_manager_user_can_view_resume( $resume->ID ) ) {
			...
			echo wp_kses_post(
				sprintf(
					...
					__( 'Your resume has been submitted successfully. To view your resume <a href="%s">click here</a>.', 'jobzilla' ),
			...
			echo '<p class="resume-submitted">';
			echo esc_html( __( 'Your resume has been submitted successfully.', 'jobzilla' ) );
			echo '</p>';
		}
		break;
	case 'pending':
		echo '<p class="resume-submitted">';
		echo esc_html( __( 'Your resume has been submitted successfully and is pending approval.', 'jobzilla' ) );
		if (
			$job_id
			...
			echo wp_kses_post(
				sprintf(
					...
					__( ' You will be able to apply for <a href="%1$s">%2$s</a> once your resume has been approved.', 'jobzilla' ),
					$job_permalink,
					$job_title
				)
			);
