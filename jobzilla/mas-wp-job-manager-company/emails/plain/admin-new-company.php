...
printf( esc_html__( 'A new company has been submitted to %1$s (%2$s).', 'jobzilla' ), esc_html( get_bloginfo( 'name' ) ), esc_url( home_url() ) );
switch ( $company->post_status ) {
	case 'publish':
		printf( ' ' . esc_html__( 'It has been published and is now available to the public.', 'jobzilla' ) );
		break;
	case 'pending':
		// translators: Placeholder %s is the admin companys URL.
		printf( ' ' . esc_html__( 'It is awaiting approval by an administrator in WordPress admin (%s).', 'jobzilla' ), esc_url( admin_url( 'edit.php?post_type=company' ) ) );
		break;
}
