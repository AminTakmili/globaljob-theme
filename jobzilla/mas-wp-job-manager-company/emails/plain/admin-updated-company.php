...
printf( esc_html__( 'A company has been updated on %1$s (%2$s).', 'jobzilla' ), esc_html( get_bloginfo( 'name' ) ), esc_url( home_url() ) );
switch ( $company->post_status ) {
	case 'publish':
		printf( ' ' . esc_html__( 'The changes have been published and are now available to the public.', 'jobzilla' ) );
		break;
	case 'pending':
		// translators: Placeholder %s is the admin companys URL.
		printf( ' ' . esc_html__( 'The company is not publicly available until the changes are approved by an administrator in the site\'s WordPress admin (%s).', 'jobzilla' ), esc_url( admin_url( 'edit.php?post_type=company' ) ) );
		break;
}
