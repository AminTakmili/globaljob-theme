<?php
	...
	if($show_column == 'col_2'){
		$col_classes = 'col-lg-6 col-md-6 m-b30 wow fadeInUp';
	}elseif($show_column == 'col_3'){
		$col_classes = 'col-lg-4 col-md-6 m-b30 wow fadeInUp';
	}else{                              
		$col_classes = 'col-lg-3 col-md-6 m-b30 wow fadeInUp';
	}
	...
	foreach ( $posts as $post ){ 		
		...
		$post_id  = $post->ID; 
		$post_title =  jobzilla_trim( $post->post_title , $title_text_limit );	
	...
	<div id="post-<?php the_ID(); ?>" <?php echo post_class($col_classes.' masonry-item'); ?> >		
		<div class="blog-post twm-blog-post-2-outer <?php echo esc_attr($post_layout); ?>" data-wow-delay=".2s" data-wow-duration="2s">			
			<?php
							...
							<div class="swiper-wrapper">
								<?php foreach($post_type_gallery1 as $image_id) { ?>
									<div class="swiper-slide">
										<img class="rounded-sm" src="<?php echo esc_url(wp_get_attachment_image_url($image_id,'medium')); ?>" alt="<?php echo esc_attr__('Image','jobzilla'); ?>">
									</div>
								<?php } ?>
							</div>
				...
				<?php if(has_post_thumbnail()) { ?>
					<div class="wt-post-media <?php echo esc_attr($media_class); ?>">
						<?php the_post_thumbnail('jobzilla_470x550') ?>
			...
			<div class="wt-post-info">
                ...
                <div class="wt-post-readmore ">
                    <a href="<?php echo esc_url(get_permalink()); ?>" class="site-button-link site-text-secondry"><?php echo esc_html__('Read More','jobzilla'); ?></a>
                </div>  
			</div>
		</div>
	</div>
